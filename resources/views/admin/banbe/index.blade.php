...
@section('main')
<p><b><h4 class="text-center" style="color:rgb(255, 67, 67)">QUẢN LÝ HÌNH ẢNH BẠN BÈ</h4></b></p>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-3">
        ...
        <button type="button" class="btn btn-outline-secondary mt-2" data-bs-toggle="modal" data-bs-target="#exampleModal">
            ...
            Thêm hình ảnh bạn bè
        </button>
    
        <form action="{{route('banbe.store')}}" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
          @csrf
    ...
    <div class="modal fade" id="xemanh{{$item->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
          <div class="modal-content">
            <div class="modal-body">
                <img src="{{url('public/uploads/banbe')}}/{{$item->avatar}}" width="100%">
            </div>
          </div>
        </div>
    </div>
        ...
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{$item->tenhinh}}</td>            
              <td >
                <img src="{{url('public/uploads/banbe')}}/{{$item->avatar}}" height="100px">
              </td>                 
              <td>{!!$item->mota!!}</td>                              
              <td class="text-right">
                ...
                <a href="{{route('banbe.edit',$item->id)}}" class="btn btn-sm btn-success">
                  <i class="fas fa-edit"></i>              
                </a> 
                <a  href="{{route('banbe.destroy',$item->id)}}" class="btn btn-sm btn-danger btndelete">
